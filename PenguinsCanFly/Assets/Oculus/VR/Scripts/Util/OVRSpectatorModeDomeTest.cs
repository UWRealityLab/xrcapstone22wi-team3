...
public class OVRSpectatorModeDomeTest : MonoBehaviour {
	...
	{
		Debug.Log("Updating spectator camera status!");
#if OVR_ANDROID_MRC
		...
		{
		Debug.Log("Please do something yeeet");
	...
	void Update () {
		Debug.Log("SAVE:Inited:" + inited + " rip");

#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN || OVR_ANDROID_MRC
		Debug.Log("We in boi8s");
		if (!inited)
		{
			Initialize();
			return;
